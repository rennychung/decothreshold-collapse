import matplotlib
matplotlib.use('TkAgg') # Use the TkAgg backend to force plot display
import numpy as np
import matplotlib.pyplot as plt

# === REALISTIC 2025–2030 PARAMETERS (DTC Cat State Simulation) ===
# Based on current optomechanical limits.
...
gamma_env = 1e4                 # s⁻¹ — environmental decoherence rate (e.g., residual gas/photon scattering)
C_th = 1e-20                    # DTC irreversibility threshold (The critical coherence value)
Gamma_0 = 1e25                  # s⁻¹ — placeholder for effectively instant pruning
...
def coherence(psi):
    # This calculation assumes the coherence is dominated by the off-diagonal terms
    # between the two cat branches. For the Gaussian/cat state model, the
    # l¹-norm of the off-diagonals is a suitable proxy for the coherence C(rho).
...
def run_dtc():
    ...
    # Q(t) = exp[-(gamma_env * Delta_x^2 / (4 * sigma_x^2)) * t]
    decay_factor = np.exp(-gamma_env * (Delta_x**2) * dt / (4*sigma_x**2))
    
    for i in range(1, steps):
        # Apply environmental decoherence analytically (decay of off-diagonal terms)
        C *= decay_factor
        
        if not triggered and C < C_th:
            # DTC TRIGGER: Instantaneous projection to one branch
        ...
        # If triggered, the coherence instantly drops to a small, non-zero value
        # due to numerical resolution, but conceptually it's zero.
...
def run_pure_decoherence():
    # Only environmental decoherence — the coherence continues to decay smoothly
    C = coherence(psi_cat)
    C_hist = [C]
    decay_factor = np.exp(-gamma_env * (Delta_x**2) * dt / (4*sigma_x**2))
...
ax1.plot(x*1e9, np.abs(psi_cat)**2, 'k--', lw=1.5, label='Initial Cat State ($|\psi_L + \psi_R|^2$)')
...
if triggered:
    # Use the determined trigger time to draw the vertical line
    ax2.axvline(t_trig, color='red', ls=':', lw=2, label=f'Instant Pruning at t≈{t_trig:.4f}s')

ax2.set_xlabel('Time (s)')
ax2.set_ylabel('Coherence $C(\\rho)$ (log scale)')
ax2.legend()
