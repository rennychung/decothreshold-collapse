...
# FIX: Adjusted line widths and plotting order one final time.
# - Dark Green (Allowed CSL) is thick and plotted first to show its immediate, dramatic split.
# - Red, Gray, and Lime Dashed lines are plotted as three distinct, parallel tracks
#   using fixed time offsets (+0.5 µs, 0, -0.5 µs) for maximum visibility.
...
# 1. 2025-allowed CSL (Dark green solid) - Plotted FIRST and thickest to ensure split is visible.
csl_allowed_lw = 4.0
...
qm_lw = 3.5 # Slightly thicker to sit visually in the middle.
...
plt.title('DTC vs CSL Coherence Decay — Parallel Tracks for Visibility', fontsize=18)
plt.legend(fontsize=13, loc='upper right')
plt.grid(True, which='both', ls='--', alpha=0.4)
plt.tight_layout()

# --- SAVE AND SHOW ---
plt.savefig('DTC_vs_CSL_PARALLEL_TRACKS_PERFECT.png', dpi=600, bbox_inches='tight')
plt.savefig('DTC_vs_CSL_PARALLEL_TRACKS_PERFECT.pdf', bbox_inches='tight')
plt.show()
