...
from qutip import *
from qutip import mesolve, basis, sigmaz, ket2dm
...
result_qm = mesolve(H, rho0, times, c_ops=c_ops_qm)
rho_qm = result_qm.states
C_qm = [2 * abs(rho[0,1]) for rho in rho_qm]

# 2. CSL Allowed (stochastic approx: white-noise phase kicks)
...
for _ in range(num_traj):
    phases = np.cumsum(np.sqrt(lambda_allowed * dt) * np.random.randn(len(times)))
    phases = phases[:len(times)]  # Fix length
    C_traj = np.exp(-gamma_env * times) * np.cos(phases)  # Approx (magnitude impact)
...
var_csl_original = lambda_original * times  # ~ lambda t approx for var

# 3. DTC (custom: conditional Lindblad approx with Euler)
rho_dtc = [rho0]
...
for i in range(1, len(times)):
    rho = rho_dtc[-1]
    # Deco step
    d_rho_deco = -0.5 * gamma_env * (L_deco * rho * L_deco.dag() - rho) * dt
    rho += d_rho_deco + d_rho_deco.dag()  # Approx Lindblad
    # DTC prune if triggered
    C = 2 * abs(rho[0,1])
    if C < C_th and not triggered:
        # Finite rate approx: rapid decay
        exp_fact = np.exp(-Gamma0 * dt)
        rho[0,1] *= exp_fact
        rho[1,0] *= exp_fact
...
print(f"  DTC C: {C_dtc[t100_idx]:.2e} (matches QM)")
